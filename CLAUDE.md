...
- **Reusable workflow izin alt-küme kuralı**: `android-build.yml` reusable'dır ve `ci.yml`'in `auto-release` job'undan `uses:` ile çağrılır. Reusable'ın istediği izinler **caller job'un izinlerinin alt kümesi** olmalı ([GitHub Docs](https://docs.github.com/en/actions/using-workflows/reusing-workflows#passing-secrets-and-assigning-permissions)). android-build.yml `actions: write` isteyince, ci.yml'deki `auto-release` job'una da `permissions: actions: write` eklemek ŞART — yoksa run `startup_failure` ile başlamadan reddedilir.
